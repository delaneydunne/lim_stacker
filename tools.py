...
def setup(mapfiles, cataloguefile, params):
    ...
    for i in range(len(mapfiles)):
        catinst = field_cull_galaxy_cat(catdict, maplist[i].fieldcent)
        catlist.append(catinst)

    return maplist, catlist

def field_cull_galaxy_cat(galdict, fieldcent, maxsep=3*u.deg):
    """
    ...
    fieldidx = fieldidx[np.where(np.logical_and(fieldz_cut > 2.35, fieldz_cut < 3.45))[0]]
